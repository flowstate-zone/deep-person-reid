...
def eval_flowstate(distmat, q_pids, g_pids, q_camids, g_camids, q_dsetids, g_dsetids, max_rank):
    ...
    for q_idx in range(num_q):
        ...
        q_pid = q_pids[q_idx]
        q_camid = q_camids[q_idx]
        q_dsetid = q_dsetids[q_idx]

        # remove gallery samples that have the same pid and camid with query or are from a different dataset
        order = indices[q_idx]
        remove = ((g_pids[order] == q_pid) & (g_camids[order] == q_camid)) | (g_dsetids[order] != q_dsetid)
        keep = np.invert(remove)
